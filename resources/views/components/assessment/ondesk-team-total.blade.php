    ...
    <div class="modal fade" id="notCompleteTeamsModal" tabindex="-1" aria-labelledby="notCompleteTeamsModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-xl">
            <div class="modal-content border-0 shadow-lg rounded-3">
                ...
                <div class="modal-body bg-light">
                    <div class="row">
                        @foreach ($categoriesDataNotComplete as $categoryName => $teams)
                            <div class="col-md-4 mb-4">
                                <div class="border-0 rounded">
                                    <p>
                                        <button class="btn btn-primary" type="button" data-bs-toggle="collapse" data-bs-target="#{{ Illuminate\Support\Str::slug(strtolower($categoryName)) }}" aria-expanded="false" aria-controls="{{ Illuminate\Support\Str::slug(strtolower($categoryName)) }}">
                                            {{ $categoryName }}
                                        </button>
                                    </p>
                                    <div class="collapse card p-2" id="{{ Illuminate\Support\Str::slug(strtolower($categoryName)) }}">
                                        @foreach ($teams as $team)
                                            <div>
                                                {{ $team->team_name }}
                                            </div>
